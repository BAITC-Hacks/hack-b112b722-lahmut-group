...
Эту часть команда проверяет независимо от настоящих моделей. Результаты с окружениями и границами приведены в [QA интерфейса](docs/QA.md) и [отчёте ML-stub стенда](docs/TESTING_WITH_ML_STUBS.md). Исторические отчёты относятся к указанным в них версиям проекта; их счётчики не являются метриками качества нового ML-профиля.
...
| Утверждение, **DOCX**, реестр и локальные напоминания | Встроенный экспорт PDF, Telegram/почта, интеграция с СЭД |
